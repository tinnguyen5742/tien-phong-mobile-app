...
const PrintingSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const printingDetailID = useRecoilValue(PrintingDetailID);
  const [typePrintingAtom, setTypePrintingAtom] = useRecoilState(
    PrintingStatusTypeAtom,
  );
  ...
  useEffect(() => {
    if (typePrintingAtom === 'EDIT' && printingDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom PrintingDetailAtom với ID:',
        printingDetailID,
      );
      handleGetPrintingDetail(printingDetailID);
      // setFormValues({
      //     ...printingAtom,
      // } as PrintingFormType);
    }
  }, [typePrintingAtom, printingDetailID]);
  ...
  const handleGetPrintingDetail = async (id: number) => {
    try {
      const url = `/machines/info/printer/${id}`;
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/printer/save`;
...
export default PrintingSettingsDetail;
