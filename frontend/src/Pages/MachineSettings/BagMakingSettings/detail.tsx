...
const BagMakingSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const bagMakingDetailID = useRecoilValue(BagMakingDetailID);
  ...
  useEffect(() => {
    if (typeBlowingAtom === 'EDIT' && bagMakingDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom BlowingDetailAtom với ID:',
        bagMakingDetailID,
      );
      handleGetBlowingDetail(bagMakingDetailID);
      // setFormValues({
      //     ...blowingAtom,
      // } as BlowingFormType);
    }
  }, [typeBlowingAtom, bagMakingDetailID]);
  ...
  const handleGoToBagMakingMachineInfo = () => {
  ...
  const handleGetBlowingDetail = async (id: number) => {
    try {
      const url = `/machines/info/bag/${id}`;
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/bag/save`;
...
export default BagMakingSettingsDetail;
