...
const LaminatingSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const laminatingDetailID = useRecoilValue(LaminatingDetailID);
  const [typeLaminatingAtom, setTypeLaminatingAtom] = useRecoilState(
    LaminatingStatusTypeAtom,
  );
  ...
  useEffect(() => {
    if (typeLaminatingAtom === 'EDIT' && laminatingDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom LaminatingDetailAtom với ID:',
        laminatingDetailID,
      );
      handleGetLaminatingDetail(laminatingDetailID);
      // setFormValues({
      //     ...laminatingAtom,
      // } as LaminatingFormType);
    }
  }, [typeLaminatingAtom, laminatingDetailID]);
  ...
  const handleGoToLaminatingMachineInfo = () => {
    if (!formValues.lsx || !formValues.tenThietBi) {
      Alert.alert('Thông báo', 'Vui lòng quét LSX và chọn máy');
    } else {
      navigate.navigate('LaminatingMachineInfo' as never);
    }
    // navigate.navigate('LaminatingMachineInfo' as never);
  };
  ...
  const handleGetLaminatingDetail = async (id: number) => {
    try {
      const url = `/machines/info/laminator/${id}`;
      // console.log("url getCaSX: ", url)
      const item = await getApi(url, {});
      console.log('handleGetLaminatingDetail: ', item);
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/laminator/save`;
...
export default LaminatingSettingsDetail;
