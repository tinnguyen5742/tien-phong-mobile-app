...
import {DepartmentModalValue, StateType, TypeFormProduce} from '../type';
...
const InputProduceNavigate = () => {
  ...
  const columns: TableColumn[] = [
    ...
    {name: 'congDoan', label: 'Công đoạn', width: 130},
    {name: 'tenCaSX', label: 'Ca'},
    {name: 'dvtGoc', label: 'DVT Gốc'},
    {name: 'slgd', label: 'SL DVT Gốc'},
  ...
  const renderCustomCell = (columnName: string, item: any) => {
    ...
    if (columnName === 'gio') {
      return (
        <Text className="text-gray-700">
          {formatTime(new Date(item.ngay), false)}
        </Text>
      );
    }
    if (columnName === 'soCT') {
    ...
    if (columnName === 'slgd') {
      return (
        <Text className="text-gray-700 font-medium">{item.headerOut.slgd}</Text>
      );
    }
    if (columnName === 'congDoan') {
      return (
        <Text className="text-gray-700 font-medium">
          {item?.headerOut?.tenCongDoan}
        </Text>
      );
    }
    if (columnName === 'dvtGoc') {
      return (
        <Text className="text-gray-700 font-medium">
          {item?.headerOut?.dvtGoc}
        </Text>
      );
    }
    if (columnName === 'sldvtGoc') {
      return (
        <Text className="text-gray-700 font-medium">
          {item?.headerOut?.sldvtGoc}
        </Text>
      );
    }
  ...
  const getList = async () => {
    ...
    try {
      ...
      const query = {pageNumber: page, pageSize: 15, loaiPhieu: 'OUT'};
  ...
  const handleEdit = (item: TypeFormProduce) => {
    ...
    navigate.navigate('OutputDetailProduce' as never);
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách sản xuất đầu ra"
        iconRight={
