...
import {BlowingFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const BlowingSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setBlowingStatusTypeAtom = useSetRecoilState(BlowingStatusTypeAtom);
  const setBlowingDetailAtom = useSetRecoilState(BlowingDetailAtom);
  const setBlowingDetailID = useSetRecoilState(BlowingDetailID);
      ...
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/blower/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/blower/list'; // Map theo đúng endpoint API list của thông số thổi
  ...
  const handleEdit = (item: any) => {
    setBlowingStatusTypeAtom('EDIT');
    // setBlowingDetailAtom(item);
    setBlowingDetailID(item.id);
    navigate.navigate('BlowingSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số thổi
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số thổi"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setBlowingStatusTypeAtom('NEW');
              setBlowingDetailAtom({} as BlowingFormType);
              navigate.navigate('BlowingSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default BlowingSettingsList;
