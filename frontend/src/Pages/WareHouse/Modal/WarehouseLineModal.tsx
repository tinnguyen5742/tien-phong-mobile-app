...
import {LineWarehouseType, WarehouseSubmitType} from '../type';
import {getApi} from '../../../Base/api/api_service';
import {AppColors} from '../../../../colors';

type WarehouseLineModalProps = {
  data: LineWarehouseType;
  handleOpenWarehouseLineModal: () => void;
  onSubmit: (data: any) => void;
  open: boolean;
  title: string;
};

const WarehouseLineModal = (props: WarehouseLineModalProps) => {
  // ✅ Bẫy lỗi 1: Trạng thái khởi tạo an toàn nếu props.slxuat hoặc props.note bị undefined
  const [dataSubmit, setDataSubmit] = useState<LineWarehouseType>(
  ...
  const handleSave = () => {
    ...
    if (dataSubmit.slXuat === '') {
      Alert.alert('Thông báo', 'Vui nhập số lượng > 0 để tiếp tục.');
      return;
    }

    const finalData = {
      ...dataSubmit,
      // Ép kiểu số để chắc chắn không bị lỗi tính toán ở file cha
    ...
    if (typeof props?.handleOpenWarehouseLineModal === 'function') {
      props.handleOpenWarehouseLineModal();
    }
  };
                ...
                <View className="pb-6 space-y-3">
                  ...
                  <View>
                    ...
                    <TextInput
                      ...
                      onChangeText={text => handleInputChange('slXuat', text)}
                    />
                  </View>
...
export default WarehouseLineModal;
