...
const SlittingSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const slittingDetailID = useRecoilValue(SlittingDetailID);
  const [typeSlittingAtom, setTypeSlittingAtom] = useRecoilState(
    SlittingStatusTypeAtom,
  );
  ...
  useEffect(() => {
    if (typeSlittingAtom === 'EDIT' && slittingDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom SlittingDetailAtom với ID:',
        slittingDetailID,
      );
      handleGetSlittingDetail(slittingDetailID);
      // setFormValues({
      //     ...slittingAtom,
      // } as SlittingFormType);
    }
  }, [typeSlittingAtom, slittingDetailID]);
  ...
  const handleGetSlittingDetail = async (id: number) => {
    try {
      const url = `/machines/info/splitter/${id}`;
      // console.log("url getCaSX: ", url)
      const item = await getApi(url, {});
      console.log('handleGetSlittingDetail: ', item);
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/splitter/save`;
...
export default SlittingSettingsDetail;
