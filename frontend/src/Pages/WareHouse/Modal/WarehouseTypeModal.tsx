...
import {TypeWarehouseModal} from '../type';

type WarehouseTypeProps = {
  handleOpenWarehouseType: () => void;
  open: boolean;
  handleChangeTypeWarehouse: (value: string) => void;
};

const WarehouseTypeModal = (props: WarehouseTypeProps) => {
  const {handleOpenWarehouseType, open, handleChangeTypeWarehouse} = props;

  const WarehouseTypeList: TypeWarehouseModal[] = [
    {
      label: 'Xuất chuyển kho',
      value: 'XUAT_CHUYEN_KHO',
    },
    // {
    //     label: 'Xuất kho',
    //     value: 'XUAT_KHO'
      ...
      <View className="flex-1 justify-center items-center bg-black/50 px-4">
        ...
        <View className="bg-white rounded-[30px] w-full max-w-sm shadow-xl overflow-hidden">
          ...
          <View className="flex-row justify-between items-center p-5 border-b border-slate-100">
            <Text className="text-lg font-bold text-slate-800">Loại phiếu</Text>
            <Pressable
          ...
          <View className="p-4 space-y-3">
            {WarehouseTypeList.map((item, index) => (
              <Pressable
                key={index}
