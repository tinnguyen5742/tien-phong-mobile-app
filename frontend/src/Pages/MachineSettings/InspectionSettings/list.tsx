...
// Import hoặc tạo Atom quản lý trạng thái tương ứng cho Thông số kiểm phẩm
// Giả định bạn tạo file store.ts tương tự bên kiểm kê
import {
  InspectionDetailAtom,
  InspectionDetailID,
  InspectionStatusTypeAtom,
} from '../store';
import {InspectionFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const InspectionSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setInspectionStatusTypeAtom = useSetRecoilState(
    InspectionStatusTypeAtom,
  );
  const setInspectionDetailAtom = useSetRecoilState(InspectionDetailAtom);
  const setInspectionDetailID = useSetRecoilState(InspectionDetailID);
  ...
  const handleDeleteItem = (item: any) => {
    Alert.alert(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa phiếu thông số kiểm phẩm ${item.soPhieu} này không?`,
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/inspector/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/inspector/list'; // Map theo đúng endpoint API list của thông số kiểm phẩm
      ...
      console.error('❌ Lỗi xảy ra tại hàm getList Thông số kiểm phẩm:', error);
      setList([]);
      setTotalPage(1);
    } finally {
      setLoadingAtom(false);
  ...
  const handleEdit = (item: any) => {
    setInspectionStatusTypeAtom('EDIT');
    // setInspectionDetailAtom(item);
    setInspectionDetailID(item.id);
    navigate.navigate('InspectionSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số kiểm kê
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số kiểm phẩm"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setInspectionStatusTypeAtom('NEW');
              setInspectionDetailAtom({} as InspectionFormType);
              navigate.navigate('InspectionSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default InspectionSettingsList;
