...
// Import hoặc tạo Atom quản lý trạng thái tương ứng cho Thông số in
// Giả định bạn tạo file store.ts tương tự bên kiểm kê
import {
  SlittingDetailAtom,
  SlittingDetailID,
  SlittingStatusTypeAtom,
} from '../store';
import {SlittingFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const SlittingSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setSlittingStatusTypeAtom = useSetRecoilState(SlittingStatusTypeAtom);
  const setSlittingDetailAtom = useSetRecoilState(SlittingDetailAtom);
  const setSlittingDetailID = useSetRecoilState(SlittingDetailID);
  ...
  const handleDeleteItem = (item: any) => {
    Alert.alert(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa phiếu thông số chia ${item.soPhieu} này không?`,
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/splitter/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/splitter/list'; // Map theo đúng endpoint API list của thông số chia
      ...
      console.error('❌ Lỗi xảy ra tại hàm getList Thông số chia:', error);
      setList([]);
      setTotalPage(1);
    } finally {
      setLoadingAtom(false);
  ...
  const handleEdit = (item: any) => {
    setSlittingStatusTypeAtom('EDIT');
    // setSlittingDetailAtom(item);
    setSlittingDetailID(item.id);
    navigate.navigate('SlittingSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số chia
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số chia"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setSlittingStatusTypeAtom('NEW');
              setSlittingDetailAtom({} as SlittingFormType);
              navigate.navigate('SlittingSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default SlittingSettingsList;
