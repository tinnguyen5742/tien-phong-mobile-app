...
import {
  PrintingDetailID,
  PrintingDetailAtom,
  PrintingStatusTypeAtom,
} from '../store';
import {PrintingFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const PrintingSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setPrintingStatusTypeAtom = useSetRecoilState(PrintingStatusTypeAtom);
  const setPrintingDetailAtom = useSetRecoilState(PrintingDetailAtom);
  const setPrintingDetailID = useSetRecoilState(PrintingDetailID);
  ...
  const handleDeleteItem = (item: any) => {
    Alert.alert(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa phiếu thông số in ${item.soPhieu} này không?`,
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/printer/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/printer/list'; // Map theo đúng endpoint API list của thông số in
      ...
      console.error('❌ Lỗi xảy ra tại hàm getList Thông số in:', error);
      setList([]);
      setTotalPage(1);
    } finally {
      setLoadingAtom(false);
  ...
  const handleEdit = (item: any) => {
    setPrintingStatusTypeAtom('EDIT');
    // setPrintingDetailAtom(item);
    setPrintingDetailID(item.id);
    navigate.navigate('PrintingSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số in
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số in"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setPrintingStatusTypeAtom('NEW');
              setPrintingDetailAtom({} as PrintingFormType);
              navigate.navigate('PrintingSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default PrintingSettingsList;
