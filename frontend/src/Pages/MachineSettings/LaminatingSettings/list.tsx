...
// Import hoặc tạo Atom quản lý trạng thái tương ứng cho Thông số thổi
// Giả định bạn tạo file store.ts tương tự bên kiểm kê
import {
  LaminatingDetailAtom,
  LaminatingDetailID,
  LaminatingStatusTypeAtom,
} from '../store';
import {LaminatingFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const LaminatingSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setLaminatingStatusTypeAtom = useSetRecoilState(
    LaminatingStatusTypeAtom,
  );
  const setLaminatingDetailAtom = useSetRecoilState(LaminatingDetailAtom);
  const setLaminatingDetailID = useSetRecoilState(LaminatingDetailID);
  ...
  const handleDeleteItem = (item: any) => {
    Alert.alert(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa phiếu thông số thổi ${item.soPhieu} này không?`,
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/laminator/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/laminator/list'; // Map theo đúng endpoint API list của thông số thổi
      ...
      console.error('❌ Lỗi xảy ra tại hàm getList Thông số thổi:', error);
      setList([]);
      setTotalPage(1);
    } finally {
      setLoadingAtom(false);
  ...
  const handleEdit = (item: any) => {
    setLaminatingStatusTypeAtom('EDIT');
    // setLaminatingDetailAtom(item);
    setLaminatingDetailID(item.id);
    navigate.navigate('LaminatingSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số laminating
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số ghép"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setLaminatingStatusTypeAtom('NEW');
              setLaminatingDetailAtom({} as LaminatingFormType);
              navigate.navigate('LaminatingSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default LaminatingSettingsList;
