...
const BlowingSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const blowingDetailID = useRecoilValue(BlowingDetailID);
  ...
  useEffect(() => {
    if (typeBlowingAtom === 'EDIT' && blowingDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom BlowingDetailAtom với ID:',
        blowingDetailID,
      );
      handleGetBlowingDetail(blowingDetailID);
      // setFormValues({
      //     ...blowingAtom,
      // } as BlowingFormType);
    }
  }, [typeBlowingAtom, blowingDetailID]);
  ...
  const handleGetBlowingDetail = async (id: number) => {
    try {
      const url = `/machines/info/blower/${id}`;
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/blower/save`;
...
export default BlowingSettingsDetail;
