...
// Import hoặc tạo Atom quản lý trạng thái tương ứng cho Thông số cắt
...
import {CuttingFormType} from './type';
import {useSafeAreaInsets} from 'react-native-safe-area-context';

const CuttingSettingsList = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setCuttingStatusTypeAtom = useSetRecoilState(CuttingStatusTypeAtom);
  const setCuttingDetailAtom = useSetRecoilState(CuttingDetailAtom);
  const setCuttingDetailID = useSetRecoilState(CuttingDetailID);
  ...
  const handleDeleteItem = (item: any) => {
    Alert.alert(
      'Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa phiếu thông số cắt ${item.soPhieu} này không?`,
      [
        {text: 'Hủy', style: 'cancel'},
        {
          ...
          onPress: async () => {
            setLoadingAtom(true);
            try {
              const url = `/machines/info/cutter/delete/${item.id}`; // Map theo đúng endpoint backend của bạn
  ...
  const getList = async () => {
    setLoadingAtom(true);
    try {
      const url = '/machines/info/cutter/list'; // Map theo đúng endpoint API list của thông số cắt
      ...
      console.error('❌ Lỗi xảy ra tại hàm getList Thông số cắt:', error);
      setList([]);
      setTotalPage(1);
    } finally {
      setLoadingAtom(false);
  ...
  const handleEdit = (item: any) => {
    setCuttingStatusTypeAtom('EDIT');
    // setCuttingDetailAtom(item);
    setCuttingDetailID(item.id);
    navigate.navigate('CuttingSettingsDetail' as never); // Điều hướng đến trang chi tiết thông số cắt
  };
  ...
  return (
    <View className="flex-1 bg-gray-100" style={{paddingBottom: insets.bottom}}>
      <HeaderComponent
        ...
        title="Danh sách Thông số cắt"
        iconRight={
          <TouchableOpacity
            onPress={() => {
              setCuttingStatusTypeAtom('NEW');
              setCuttingDetailAtom({} as CuttingFormType);
              navigate.navigate('CuttingSettingsDetail' as never);
            }}
            className="p-2">
            <FontAwesomeIcon
              icon={faPlus}
...
export default CuttingSettingsList;
