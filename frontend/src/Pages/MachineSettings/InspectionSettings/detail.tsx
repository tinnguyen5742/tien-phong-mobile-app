...
const InspectionSettingsDetail = () => {
  const navigate = useNavigation();
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const inspectionDetailID = useRecoilValue(InspectionDetailID);
  const [typeInspectionAtom, setTypeInspectionAtom] = useRecoilState(
    InspectionStatusTypeAtom,
  );
  ...
  useEffect(() => {
    if (typeInspectionAtom === 'EDIT' && inspectionDetailID) {
      console.log(
        '🚀 Đang ở chế độ EDIT, tiến hành load dữ liệu chi tiết từ Atom InspectionDetailAtom với ID:',
        inspectionDetailID,
      );
      handleGetInspectionDetail(inspectionDetailID);
      // setFormValues({
      //     ...inspectionAtom,
      // } as InspectionFormType);
    }
  }, [typeInspectionAtom, inspectionDetailID]);
  ...
  const handleGetInspectionDetail = async (id: number) => {
    try {
      const url = `/machines/info/inspector/${id}`;
      // console.log("url getCaSX: ", url)
      const item = await getApi(url, {});
      console.log('handleGetInspectionDetail: ', item);
  ...
  const handleSave = async (status: string) => {
    ...
    // return
    try {
      const url = `/machines/info/inspector/save`;
...
export default InspectionSettingsDetail;
