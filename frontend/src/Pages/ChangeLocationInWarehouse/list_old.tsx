...
const ChangeLocationInWarehouse = () => {
  const setLoadingAtom = useSetRecoilState(loadingStore);
  const setChangeLocationWarehouseStatusTypeAtom = useSetRecoilState(
    ChangeLocationWarehouseStatusTypeAtom,
  );
  const setChangeLocationWarehouseDetailAtom = useSetRecoilState(
    ChangeLocationWarehouseDetailAtom,
  );
  const setChangeLocationWarehouseDetailID = useSetRecoilState(
    ChangeLocationWarehouseDetailID,
  );

  const navigate = useNavigation();

  const [list, setList] = useState<TypeFormChangeLocationWarehouse[]>([]);
  const [page, setPage] = useState(1);
  const [totalPage, setTotalPage] = useState(0);
  const defaultChangeLocationWarehouseValue: TypeFormChangeLocationWarehouse = {
    soCT: '',
    ngay: new Date().toISOString(),
    phieuYC: '', // 🌟 BỔ SUNG: Trường bắt buộc thiếu trong object cũ
    ghiChu: '',
    maKhoXuat: '',
    khoXuat: '',
    viTriXuat: '',
    ...
    khoNhap: '',
    viTriNhap: '',
    TinhTrang: 'draft', // 🌟 SỬA: Viết HOA chữ T cho đúng với định nghĩa Type của bạn
    User: '',
  ...
  const renderCustomCell = (columnName: string, item: any) => {
    ...
    if (columnName === 'ghiChu') {
      return (
        <View style={{width: 200}} className="items-start justify-center px-2">
  ...
  const handleDeleteItem = (item: TypeFormChangeLocationWarehouse) => {
    Alert.alert(
      'Xác nhận xóa',
  ...
  const handleEdit = (item: TypeFormChangeLocationWarehouse) => {
    setChangeLocationWarehouseStatusTypeAtom('EDIT');
    // setChangeLocationWarehouseDetailAtom(item);
    setChangeLocationWarehouseDetailID(item.id || 0);
    navigate.navigate('ChangeLocationInWarehouseDetail' as never);
  };
      ...
      <HeaderComponent
        ...
        iconRight={
          <View className="flex-row items-center space-x-1">
            ...
            <TouchableOpacity
              onPress={() => {
                setChangeLocationWarehouseStatusTypeAtom('NEW');
                setChangeLocationWarehouseDetailAtom(
                  defaultChangeLocationWarehouseValue,
                );
                navigate.navigate('ChangeLocationInWarehouseDetail' as never);
              }}
              className="p-2 active:opacity-60">
...
export default ChangeLocationInWarehouse;
